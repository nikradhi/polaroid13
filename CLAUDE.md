...
The 3-shot photobooth ([js/photobooth.js](js/photobooth.js)) composites three square canvas shots into one vertical strip with the couple's name + date drawn in the footer, then feeds it through the *same* `compressImej` → base64 → Firestore path as a normal upload. It passes its own `{lebarMaks: LEBAR_JALUR, sasaranBait: SASARAN_JALUR}` (480 px / 45 KB, declared in [js/imej.js](js/imej.js)) so the strip stores at **~41–57 KiB — smaller than a normal photo** despite holding three shots. Never widen these without redoing the quota arithmetic.

**Strip geometry is per-frame, not constant.** [js/bingkai.js](js/bingkai.js) `geometriJalur(padding)` derives everything from the chosen frame's margin: "Klasik" keeps `padding: 16` → 448 px photos, 1480 tall (byte-identical to strips sent before frames existed); decorated frames use `padding: 34` → 412 px photos, **1390 tall — shorter, because square photos shrink faster than the margin grows**. Shots are always captured at the largest size (`SISI_TANGKAP` 448) and scaled down at draw time, so switching frames never needs a re-shoot and never costs quality.

**Drawing is split from compression, deliberately.** `lukisJalur(idBingkai)` returns a canvas synchronously (~40 ms); `mampatJalur(canvas)` runs `compressImej` (~1.8 s) and is called **only on submit**. Fusing them back together would make every frame tap take 1.8 s and the chooser would feel broken. The preview `<img>` comes from a plain `canvas.toBlob()`, never `compressImej`.

Frames are **canvas vector art tinted with `warnaTema()`** — never image files, never SVG data-URIs. Drawing an external `<image>` into the canvas *taints* it and `toBlob()` then throws `SecurityError`, killing the whole strip. `lukisKaki()`'s line—♥—line divider is the original precedent. Frame line widths are multiplied by a module-level `ganda` inside `sediaGaris()`/`tebalGaris()` so the ~96 px chooser tiles stay legible; it is reset in a `finally` — if it leaks, real strips get comically thick borders.

Three non-obvious constraints hold this together:

- **`compressImej`'s width ladder collapses at `lebarMaks <= 480`.** `lebarCubaan = [lebarMaks, 640, 560, 480].filter(...)` leaves a *single* width, so quality can bottom out at q0.48 with the strip never shrinking. `mampatJalur()` therefore runs a **second pass at 400 px** when the first result exceeds `SASARAN_JALUR * 1.35`. Remove that guard and noisy/dark strips balloon.
- **Frame art costs quota.** Sparse line art in the margin is cheap; a full-bleed texture would raise JPEG entropy and degrade the *whole* strip, not just the border. Measured across all 11 frames: 41.3–56.5 KiB stored, all under the 95 KiB skip threshold. Re-measure after adding a frame — a busy one can wipe out the headroom.
...
- **Anti-spam** in upload.js is client-side only (honeypot field) — bypassable, not a real control. There is deliberately no upload cooldown: guests are expected to send several photos in a row, and the server-enforced `photoLimit` quota is the real brake.
...
- **Two password-reset paths in the super-admin panel, and they are not interchangeable.** The card's **"Reset kata laluan"** button is `sendPasswordResetEmail()` — zero-backend, the customer sets their own password from the emailed link; it is the correct default. The **"KL baharu" field + "Set"** button takes effect immediately, for customers who cannot reach their email. Since the client SDK cannot change another user's password, it signs in *as that customer* on a **second Firebase app instance** (`tukarKataLaluanPelanggan()` in [js/firebase.js](js/firebase.js), same trick as `ciptaAkaunPelanggan()`) and calls `updatePassword()` — the admin's own session on the main `auth` is untouched. That needs the current password, so account creation records it in **`eventsPrivate/{id}.kataLaluan`** (plaintext, admin-only per `firestore.rules`). Never move that field to `events` — that doc is world-readable. The recorded password goes stale if the customer uses the email link; the sign-in then fails with `auth/invalid-credential` and `mesejRalatKl()` points the admin back at the email button. Events created before this feature have no recorded password and fall back to a `prompt()` for the current one.
